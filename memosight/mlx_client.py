...
Supports describe(), answer(), analyze_batch(), and health() operations.
...
class MlXVlmClient:
    ...
    async def analyze_batch(
        ...
        Args:
            image_paths: List of file paths to analyze.
            task: One of "describe", "quality", or a custom prompt string.
            progress_callback: Optional sync callback called for batch/image stages.

        Returns:
            list of {"path": str, "observation": str, "error": str|None}
        """
        task_map = {
            "describe": self.describe,
            "quality": self.analyze_quality,
        }
        handler = task_map.get(task) if task in task_map else None
            ...
            progress_callback(
                {
                    ...
                    "task": task,
                    "model": model_id,
                    "duration_s": time.perf_counter() - model_started_at,
                }
            )
        ...
        for index, path in enumerate(image_paths, start=1):
            ...
            try:
                if handler:
                    if task == "describe":
                        obs = await handler(path, language=language)
                    else:
                        obs = await handler(path)
                else:
                    obs = await self._chat_with_image(
                        image_path=path,
                        system_prompt=task,
                        user_text="Analyze this image.",
                    )
                duration_s = time.perf_counter() - image_started_at
                finish = self.finish_reason(obs)
                truncated = finish["truncated"]
                output_ok = not truncated
                results.append(
                    {
                        "path": path,
                        "observation": obs,
                ...
                window_durations.append(duration_s)
                ...
                if isinstance(completion_tokens, int):
                    window_completion_tokens += completion_tokens
            ...
            if progress_callback and (index % 10 == 0 or index == total):
                progress_callback(
                    {
                        "status": "batch_window_summary",
                        "task": task,
                        "language": language,
                        "current": index,
                        "total": total,
                        "window_size": len(window_durations),
                        "window_avg_duration_s": (
                            sum(window_durations) / len(window_durations)
                            if window_durations
                            else 0.0
                        ),
                        "window_ok": window_ok,
                        ...
                        "cumulative_truncated": cumulative_truncated,
                        "cumulative_failed": cumulative_failed,
                        "window_total_tokens": window_total_tokens,
                        "window_completion_tokens": window_completion_tokens,
            ...
            progress_callback(
                {
                    ...
                    "language": language,
                    "total": total,
                    "duration_s": time.perf_counter() - batch_started_at,
                }
            )
        return results
