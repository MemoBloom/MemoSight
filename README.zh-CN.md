...
- **小模型可用的两段式方案。** 将视觉理解拆分为「图片→caption」和
  「caption→字段」两次调用——在小型本地 VLM 上约快 1.7 倍且更可靠
  （见基准测试），第二阶段可独立重试。
...
`mlx_vlm.server` 提供服务。9 个短视频（美食、旅行、开箱、vlog、教程），
每个视频均匀抽取 10 帧，共 90 帧，预热后交替执行顺序。

| 视频 | 一段式 JSON | 两段式 | 加速比 |
| --- | ---: | ---: | ---: |
| 吃播（热狗 + 芝士） | 3.87s | 2.30s | **1.68x** |
| 迪士尼片段 | 3.89s | 1.99s | **1.95x** |
| 17 分钟购物分享 | 6.01s | 3.38s | **1.78x** |
| 古巴旅行纪录片 | 6.30s | 3.87s | **1.63x** |
| 美妆开箱 | 6.22s | 3.83s | **1.62x** |
| 螺蛳粉美食 vlog | 6.36s | 3.59s | **1.77x** |
| 韩国跑腿 vlog | 6.48s | 4.03s | **1.61x** |
| 澳洲旅行 vlog | 5.09s | 3.05s | **1.67x** |
| 深蹲教程 | 5.26s | 3.36s | **1.56x** |
| **全部 90 帧** | **平均 5.50s** | **平均 3.27s** | **1.68x** |

- 两段式的拆分开销很小：caption ≈ 1.55s，字段抽取 ≈ 1.72s。
- 可靠性：一段式 90/90 `ok`；两段式 90/90 `ok`——零 `partial`、零硬
  失败。默认 profile 的完整输出 Markdown 契约（恰好 7 个字段行、必须以
  `search_tags` 行结尾）消除了旧模板的提前停止故障。即便第二阶段偶发
  问题，caption 仍会返回，用 `extract_fields(caption)` 重试即可，无需
  再次处理图片。
- 帧数据集在 `test_data/`（即基准测试所用的每个视频 10 帧）。原始数据
  和可交互的逐帧对比评审页在本地生成——
  `scripts/run_test_data_compare.py` 输出
  `results/test_data_compare_one_vs_two_stage.json`，
